...
public class  objectDetectorClass {
    ...
    private  int width=0;
    private int Classifcation_Input_Size=0;
    ...
    objectDetectorClass(AssetManager assetManager,String modelPath, String labelPath,int inputSize,String classification_model,int classification_input_size) throws IOException{
        ...
        Classifcation_Input_Size=classification_input_size;
      ...
      //  float[][][]result=new float[1][10][4];
        ...
        for (int i=0;i<10;i++){
            ...
            if(score_value>0.5){
                ...
                //now set height & width of the box
                float w1=x2-x1;
                float h1=y2-y1;
                ...
                Bitmap scaledBitmap1=Bitmap.createScaledBitmap(bitmap1,Classifcation_Input_Size,Classifcation_Input_Size,false);
                // Convert scaledBitmap to byte buffer
                ByteBuffer byteBuffer1=convertBitmapToByteBuffer1(scaledBitmap1);
                //create array for output of interpreter2
                ...
                // convert class output_class_value to alphabetes
    ...
    private ByteBuffer convertBitmapToByteBuffer1(Bitmap bitmap) {
        ...
        int quant=1;
        // Change input size
        int size_images=Classifcation_Input_Size;
        if(quant==0){
            byteBuffer=ByteBuffer.allocateDirect(1*size_images*size_images*3);
        }
        else {
        ...
        bitmap.getPixels(intValues,0,bitmap.getWidth(),0,0,bitmap.getWidth(),bitmap.getHeight());
        int pixel=0;
        // remove 255.0f because we didn't scale the image
        for (int i=0;i<size_images;++i){
            for (int j=0;j<size_images;++j){
                final  int val=intValues[pixel++];
                if(quant==0){
